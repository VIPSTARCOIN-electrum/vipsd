...
// newAddressPubKey returns a new vipsutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) vipsutil.Address {
	addr, err := vipsutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new vipsutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) vipsutil.Address {
	addr, err := vipsutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new vipsutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) vipsutil.Address {
	addr, err := vipsutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the vipsutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the vipsutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := vipsutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := vipsutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := vipsutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := vipsutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := vipsutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*vipsutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*vipsutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*vipsutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*vipsutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*vipsutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
