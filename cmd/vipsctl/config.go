...
var (
	vipsdHomeDir           = vipsutil.AppDataDir("vipsd", false)
	vipsctlHomeDir         = vipsutil.AppDataDir("vipsctl", false)
	vipswalletHomeDir      = vipsutil.AppDataDir("vipswallet", false)
	defaultConfigFile     = filepath.Join(vipsctlHomeDir, "vipsctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(vipsdHomeDir, "rpc.cert")
	defaultWalletCertFile = filepath.Join(vipswalletHomeDir, "rpc.cert")
)
...
// config defines the configuration options for vipsctl.
//
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(vipsctlHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		// Use config file for RPC server to create default vipsctl config
		var serverConfigPath string
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(vipswalletHomeDir, "vipswallet.conf")
		} else {
			serverConfigPath = filepath.Join(vipsdHomeDir, "vipsd.conf")
		}
...
// For this it tries to read the config file for the RPC server (either vipsd or
// vipswallet), and extract the RPC user and password from it.
