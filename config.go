...
import (
	...
	"github.com/VIPSTARCOIN-electrum/vipsd/connmgr"
...
const (
	defaultConfigFilename        = "vipsd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "vipsd.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-vipsd.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = vipsutil.AppDataDir("vipsd", false)
...
// config defines the configuration options for vipsd.
//
...
type config struct {
	...
	Listeners            []string      `long:"listen" description:"Add an interface/port to listen for connections (default all interfaces port: 31915, testnet: 32915)"`
	...
	RPCListeners         []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 31914, testnet: 32914)"`
...
//
// The above results in vipsd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableRPC {
		vipsdLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = vipsutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]vipsutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := vipsutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if configFileError != nil {
		vipsdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-vipsd.conf to the given destination path,
...
// vipsdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func vipsdDial(addr net.Addr) (net.Conn, error) {
...
// vipsdLookup resolves the IP of the given host using the correct DNS lookup
...
func vipsdLookup(host string) ([]net.IP, error) {
