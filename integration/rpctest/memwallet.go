...
import (
	...
	"github.com/VIPSTARCOIN-electrum/vipsutil/hdkeychain"
)
...
type memWallet struct {
	...
	addrs map[uint32]vipsutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]vipsutil.Address)
...
// SetRPCClient saves the passed rpc connection to vipsd as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*vipsutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          vipsutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (vipsutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []vipsutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt vipsutil.Amount,
	feeRate vipsutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := vipsutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	...
	var outputAmt vipsutil.Amount
	for _, output := range outputs {
		outputAmt += vipsutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() vipsutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance vipsutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (vipsutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := vipsutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
