...
// Harness fully encapsulates an active vipsd process to provide a unified
// platform for creating rpc driven integration tests involving vipsd. The
// active vipsd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active vipsd process is fully
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*vipsutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the vipsd node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []vipsutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure vipsd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created vipsd
...
func (h *Harness) NewAddress() (vipsutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() vipsutil.Amount {
...
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut,
	feeRate vipsutil.Amount) (*chainhash.Hash, error) {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate vipsutil.Amount, change bool) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*vipsutil.Tx, blockVersion int32,
	blockTime time.Time) (*vipsutil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*vipsutil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*vipsutil.Block, error) {
	...
	prevBlock := vipsutil.NewBlock(mBlock)
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "vipsd", "rpctest")
