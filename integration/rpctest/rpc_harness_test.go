...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt vipsutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(vipsutil.Amount(5 * vipsutil.SatoshiPerBitcoin))
	...
	txid = genSpend(vipsutil.Amount(500 * vipsutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(vipsutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*vipsutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, vipsutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := vipsutil.Amount(250 * vipsutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = vipsutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := vipsutil.Amount(50 * vipsutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := vipsutil.Amount(numMatureOutputs * 50 * vipsutil.SatoshiPerBitcoin)
