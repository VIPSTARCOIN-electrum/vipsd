...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	vipsdPath, err := vipsdExecutablePath()
	if err != nil {
		vipsdPath = "vipsd"
	}
...
// arguments returns an array of arguments that be used to launch the vipsd
// process.
...
// command returns the exec.Cmd which will be used to start the vipsd process.
...
// to the vipsd process that is launched via Start().
...
// as the base for the log and data directories for vipsd.
...
// start creates a new vipsd process, and writes its pid in a file reserved for
...
// stop interrupts the running vipsd process process, and waits until it exits
...
// shutdown terminates the running vipsd process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := vipsutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
