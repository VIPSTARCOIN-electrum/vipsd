...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: vipsutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := vipsutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*vipsutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, vipsutil.NewTx(tx))
	}
...
func additionalCoinbase(amount vipsutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee vipsutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee vipsutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee vipsutil.Amount) *wire.MsgTx {
