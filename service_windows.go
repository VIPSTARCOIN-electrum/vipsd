...
const (
	// svcName is the name of vipsd service.
	svcName = "vipsdsvc"
...
// logServiceStartOfDay logs information about vipsd when the main server has
...
// vipsdService houses the main service handler which handles all service
// updates and launching vipsdMain.
type vipsdService struct{}
...
// long-running vipsdMain (which is the real meat of vipsd), handles service
// change requests, and notifies the service control manager of changes.
func (s *vipsdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start vipsdMain in a separate goroutine so the service can start
...
// installService attempts to install the vipsd service.  Typically this should
...
// removeService attempts to uninstall the vipsd service.  Typically this should
...
// startService attempts to start the vipsd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &vipsdService{})
	if err != nil {
