...
const (
	...
	MinHighPriority = vipsutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr vipsutil.Address) (*vipsutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *vipsutil.Tx, height int32) error {
...
func logSkippedDeps(tx *vipsutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress vipsutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*vipsutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := vipsutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := vipsutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A vipsutil.Block should use saved in the state to avoid
