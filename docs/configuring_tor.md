...
vipsd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, vipsd supports a hybrid,
...
Configuring vipsd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the vipsd command
line or in the vipsd configuration file.  Typically the Tor proxy address will be
...
By default, vipsd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since vipsd will act as the server.
...
```bash
$ ./vipsd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
vipsd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the vipsd command line or in the vipsd
...
```bash
$ ./vipsd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
vipsd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./vipsd --proxy=127.0.0.1:9050 --torisolation
```
