...
import (
	...
	"github.com/VIPSTARCOIN-electrum/vipsd/mining/cpuminer"
	...
	"github.com/VIPSTARCOIN-electrum/vipsutil/bloom"
)
...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a vipsutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := vipsutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a vipsutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := vipsutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > vipsutil.MaxSatoshi {
		peerLog.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, vipsutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
func (s *server) TransactionConfirmed(tx *vipsutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			vipsdLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new vipsd server configured to listen on addr for the
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params, interrupt <-chan struct{}) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, vipsdLookup)
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *vipsutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
