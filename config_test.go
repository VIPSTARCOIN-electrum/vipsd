...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-vipsd.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "vipsd")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of vipsd binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-vipsd.conf")
