...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *vipsutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *vipsutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *vipsutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a vipsutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := vipsutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a vipsutil.Address) bool {
	switch a := a.(type) {
	case *vipsutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a vipsutil.Address) {
	switch a := a.(type) {
	case *vipsutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected vipsutil.Block
type notificationBlockDisconnected vipsutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *vipsutil.Tx,
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *vipsutil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *vipsutil.Tx) {
	...
	ntfn := btcjson.NewTxAcceptedNtfn(txHashStr, vipsutil.Amount(amount).ToBTC())
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*vipsutil.Tx)
	for _, op := range ops {
...
func blockDetails(block *vipsutil.Block, txIndex int) *btcjson.BlockDetails {
	if block == nil {
		return nil
	}
...
func newRedeemingTxNotification(txHex string, index int, block *vipsutil.Block) ([]byte, error) {
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *vipsutil.Tx, block *vipsutil.Block) {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *vipsutil.Tx,
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *vipsutil.Tx, block *vipsutil.Block) {
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := vipsutil.DecodeAddress(addr, params)
		if err != nil {
...
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *vipsutil.Block) {
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *vipsutil.Block, params *chaincfg.Params) []string {
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *vipsutil.Block) error {
...
fetchRange:
	...
	// Notify websocket client of the finished rescan.  Due to how vipsd
