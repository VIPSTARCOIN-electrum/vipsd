...
const (
	...
	DefaultMinRelayTxFee = vipsutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee vipsutil.Amount) int64 {
	...
	if minFee < 0 || minFee > vipsutil.MaxSatoshi {
		minFee = vipsutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *vipsutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee vipsutil.Amount) bool {
...
func checkTransactionStandard(tx *vipsutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee vipsutil.Amount,
...
func GetTxVirtualSize(tx *vipsutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
