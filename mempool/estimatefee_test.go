...
func (eft *estimateFeeTester) testTx(fee vipsutil.Amount) *TxDesc {
	eft.version++
	return &TxDesc{
		TxDesc: mining.TxDesc{
			Tx: vipsutil.NewTx(&wire.MsgTx{
				Version: eft.version,
			}),
			Height: eft.height,
			Fee:    int64(fee),
...
func (eft *estimateFeeTester) newBlock(txs []*wire.MsgTx) {
	...
	block := vipsutil.NewBlock(&wire.MsgBlock{
		Transactions: txs,
	})
...
func (eft *estimateFeeTester) round(txHistory [][]*TxDesc,
	...
	for i := uint32(0); i < txPerRound; i++ {
		newTx := eft.testTx(vipsutil.Amount(rand.Intn(1000000)))
