...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *vipsutil.Tx, details *btcjson.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *vipsutil.Tx, details *btcjson.BlockDetails)
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount vipsutil.Amount)
	...
	// such as btcwallet.
	OnAccountBalance func(account string, balance vipsutil.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	case btcjson.BtcdConnectedNtfnMethod:
		...
		if err != nil {
			log.Warnf("Received invalid vipsd connected "+
				"notification: %v", err)
			return
		}
...
//
// NOTE: This is a vipsd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*vipsutil.Tx, error) {
	...
	transactions := make([]*vipsutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = vipsutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*vipsutil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return vipsutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := vipsutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
// parseBtcdConnectedNtfnParams parses out the connection status of vipsd
// and vipswallet from the parameters of a vipsdconnected notification.
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance vipsutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := vipsutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: This is a vipsd extension and requires a websocket connection.
...
//
// NOTE: Deprecated. Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []vipsutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
...
//
// NOTE: Deprecated. Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []vipsutil.Address) error {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []vipsutil.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a vipsd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []vipsutil.Address,
...
// NOTE: This is a vipsd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []vipsutil.Address, outPoints []wire.OutPoint) error {
