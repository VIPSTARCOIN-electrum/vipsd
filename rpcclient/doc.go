...
In addition to the compatible standard HTTP POST JSON-RPC API, vipsd and
...
vipsd or btcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by vipsd and
...
All notifications provided by vipsd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by vipsd (and
...
Also, it is important to realize that vipsd intentionally separates the wallet
functionality into a separate process named btcwallet.  This means if you are
connected to the vipsd RPC server directly, only the RPCs which are related to
...
The following full-blown client examples are in the examples directory:
 ...
 - btcdwebsockets
   Connects to a vipsd RPC server using TLS-secured websockets, registers for
